    ...
    if (entity === 'all' || entity === 'orders') {
      try {
        ...
        if (orders.length > 0) {
          ...
          const { error } = await supabaseAdmin.from('orders').upsert(data, { onConflict: 'tenant_id,external_id' });
          if (error && error.code === '42P10') {
            const ids = data.map((o: any) => o.external_id);
            await supabaseAdmin.from('orders').delete().eq('tenant_id', tenantId).in('external_id', ids);
            const { error: ie } = await supabaseAdmin.from('orders').insert(data);
            if (ie) results.errors.push('Pedidos: ' + ie.message);
            else results.orders = data.length;
          } else if (error) {
