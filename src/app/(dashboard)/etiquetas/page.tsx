...
export default function EtiquetasPage() {
  ...
  const [clients, setClients] = useState<LabelClient[]>([]);
  ...
  const loadLabels = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await authFetch('/api/v2/labels/export');
      if (res.ok) {
        const json = await res.json() as { data: LabelSummary[] };
        setLabels(json.data || []);
      }
    } catch { /* silencioso */ }
  ...
  const loadClients = useCallback(async (label: string) => {
    setClientsLoading(true);
    try {
      const res = await authFetch(`/api/v2/labels/export?label=${encodeURIComponent(label)}`);
      if (res.ok) {
        const json = await res.json() as { data: LabelClient[] };
  ...
  useEffect(() => {
    if (selectedLabel) {
      loadClients(selectedLabel);
    } else {
      setClients([]);
    }
  }, [selectedLabel, loadClients]);
  ...
  function handleExportCSV(labelName: string) {
    const token = useAuthStore.getState().accessToken;
    const url = `/api/v2/labels/export?label=${encodeURIComponent(labelName)}&format=csv`;
    ...
    fetch(url, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      ...
      .then(blob => {
        ...
        a.download = `etiqueta-${labelName}.csv`;
        a.click();
      })
